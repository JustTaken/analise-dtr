...
\begin{document}
	...
	\begin{center}
		{\bfseries\fontsize{14}{16}\selectfont CONTRIBUIÇÕES SOBRE O USO DE UM APARATO EXPERIMENTAL COM APLICAÇÕES NO BENEFICIAMENTO DE MINÉRIO}
	\end{center}
	...
	\begin{multicols}{2}
		...
		\section{Montagem do sistema}
			A montagem do sistema é baseada na presença de quatro bombas de diafragma para transporte da água de diluição, duas bombas peristalticas para injeção do traçador em pulso de $0.04$ s, dois tanques de água, um tanque com balança acoplada, um tanque de mistura, duas valvulas solenoide que leva água ou para o tanque reservatório interno ou para o tanque de recalque e um medidor de concentração na saida do misturador, a Figura \ref{fig:system} exemplifica a montagem do sistema.

			\begin{figure}[H]
				\centering
				\includegraphics[width=\columnwidth]{image/system.png}
				\caption{Esquemático da montagem do sistema.}
				\label{fig:system}
			\end{figure}

			Segundo essa configuração, a medição da concentração no misturador funciona como um vaso fechado, ou seja, uma caixa preta e que o estudo da fluiodinâmica é inteiramente baseado na interpretação dos dados de concentração de saida, isso em função do posicionamento ou geometria dos elementos internos.
		...
		\section{Tratamento de Dados}
			...
			A precisão dos resultados é incrementada pela expansão dos valores adjacentes ao mínimo encontrado. Tanto para N quanto para tau, a diminuição da diferença entre valores sucessivos é dada por um fator de $100$, isto é feito quatro vezes, porque a partir dessa quantidade de iterações o progresso é notavelmente pequeno.
		...
		\section{Resultados}
			...
			\begin{figure}[H]
				...
				\caption{DTR para o primeiro teste do planejamento experimental e placa com furo central como elemento interno de mistura.}
			...
			\begin{figure}[H]
				...
				\caption{DTR para o primeiro teste do planejamento experimental e cone voltado para baixo como elemento interno de mistura.}
			...
			\begin{figure}[H]
				...
				\caption{DTR para o primeiro teste do planejamento experimental e cone voltado para cima como elemento interno de mistura.}
				\label{fig:up_raw_curve}
			\end{figure}

			Pode-se ver, ainda nos primeiros experimentos que existe uma diferênça nas dispersões axiais de cada uma das configurações. Isto é visto a partir da analise da variância em uma analise quantitativa, ou a partir da observação da largura da curva, em uma analise visual de proximidade com um comportamento de reator CSTR. A ordem de dispersão axial por padrões visuais da maior para a menor é placa, cone para cima e cone para baixo respectivamente.
